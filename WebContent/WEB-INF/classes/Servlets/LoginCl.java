...
public class LoginCl extends HttpServlet {
	...
	Logger logger=LogManager.getLogger(LoginCl.class);//获取logger, error>warn>info, info会打印出所有error,warn和info的信息
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if(rs.next())
			{
				 ...
				 * Domino登录
				 */
				if(DominoLogin.Login(uname, rPassword))
				{
					ret="1";//Login Success!
				  ...
				  if(num<5){
					...
					boolean b=update.updateUser(uname);//更新用户锁定状态
					boolean c=update.UpdateTime(uname);	//更新用户锁定时间
					ret="-1";
					if (b==ture) {
